...
def _serialize_store_memberships(instance):
    return [
        {
        ...
        for membership in instance.store_memberships.select_related("store", "role").all()
    ]


def _sync_store_memberships(user, stores_data):
    ...
    for item in stores_data:
        ...
        role_code = item.get("role") or item.get("role_code")
        try:
            store_id = int(store_id)
        except (TypeError, ValueError):
            raise serializers.ValidationError(
                {"stores": _("Magasin invalide pour l'utilisateur.")}
            )
        if store_id in submitted_store_ids:
            raise serializers.ValidationError(
                {"stores": _("Un magasin ne peut être affecté qu'une seule fois.")}
            )
        ...
        try:
            ...
            raise serializers.ValidationError(
                {"stores": _("Rôle magasin invalide pour l'utilisateur.")}
            )
        ...
        submitted_store_ids.append(store_id)
...
class UsersListSerializer(serializers.ModelSerializer):
    ...
    @staticmethod
    def get_stores(instance):
        return _serialize_store_memberships(instance)

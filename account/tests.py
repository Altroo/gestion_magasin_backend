...
class TestAccountAPIExtras:
    ...
    def test_post_users_create_with_store_membership(self):
        store = Store.objects.create(name="Test Store", code="TEST_STORE")
        role, _ = Role.objects.get_or_create(
            code=Role.Codes.VENDEUR,
            defaults={"name": "Vendeur", "rank": 3},
        )
        url = reverse("account:users")
        payload = {
            ...
            "is_staff": False,
            "is_active": True,
            "stores": [{"store_id": store.id, "role": role.code}],
        }

        resp = self.auth_client.post(url, payload, format="json")

        assert resp.status_code == status.HTTP_204_NO_CONTENT
        created = self.user_model.objects.get(email="storeuser@example.com")
        membership = StoreMembership.objects.get(user=created, store=store)
        assert membership.role == role
        assert membership.is_active is True

    def test_user_detail_put_updates_store_memberships(self):
        target = self.user_model.objects.create_user(
            email="storeedit@example.com",
            password="securepass123",
            first_name="Store",
            last_name="Edit",
            is_staff=False,
        )
        old_store = Store.objects.create(name="Old Store", code="OLD_STORE")
        new_store = Store.objects.create(name="New Store", code="NEW_STORE")
        ...
        StoreMembership.objects.create(user=target, store=old_store, role=old_role)

        url = reverse("account:users_detail", args=[target.pk])
        resp = self.auth_client.put(
            url,
            {
                "first_name": target.first_name,
                "last_name": target.last_name,
                "stores": [{"store_id": new_store.id, "role": new_role.code}],
            },
            format="json",
        )

        assert resp.status_code == status.HTTP_200_OK
        assert not StoreMembership.objects.filter(user=target, store=old_store).exists()
        membership = StoreMembership.objects.get(user=target, store=new_store)
